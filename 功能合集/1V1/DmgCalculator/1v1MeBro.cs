...
namespace SpellNamesandSlots
{
    internal class Program
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            try
            {
                menu = new Menu("1v1战个痛", "dmg", true);
                menu.AddItem(
                    new MenuItem("range", "检测范围").SetValue(new Slider(1500, 300, 20000)));
                menu.AddItem(
                    new MenuItem("gthan2", "如果检测到敌人CD > 2  (自己)").SetValue(new Slider(2, 0, 8)));
                menu.AddItem(
                    new MenuItem("lthan2", "如果检测到敌人CD <= 2 (自己)").SetValue(new Slider(1, 0, 8)));
                menu.AddItem(
                    new MenuItem("theircd", "如果他们检测到他们CD <= 2 敌人").SetValue(new Slider(4, 0, 10)));
                menu.AddItem(
                    new MenuItem("theircd2", "检测到他们的CD (>2)").SetValue(new Slider(4, 0, 10)));
                menu.AddToMainMenu();
